...
public:
  ...
  KDTreeFlann(Eigen::MatrixXd &data, const bool copy = false) : KDTreeFlann() {
    setData(data, copy);
  }
  ...
  void setData(Eigen::MatrixXd &data, const bool copy = false) {
    if (copy) {
      dataCopy_ = data;
  ...
  void query(Eigen::MatrixXd &queryPoints, const size_t knn, Eigen::MatrixXi &indices,
    ...
    if (dataPoints_->rows() != queryPoints.rows())
      throw std::runtime_error("cannot query KDTree; KDTree has different "
                               "dimension than query data");
    ...
    flann::Matrix<Scalar> queryPts(queryPoints.data(), queryPoints.cols(),
                                   queryPoints.rows());
    flann::Matrix<int> indicesF(indices.data(), indices.cols(), indices.rows());
